...
def load_dinov2_backbone(model_base_name: str) -> torch.nn.Module:
    """Load a DINOv2 backbone from the locally installed package.

    This avoids relying on ``torch.hub`` reaching GitHub at runtime.
    The installed ``dinov2`` package still handles pretrained weight downloads.
    """

    try:
        model_builder = dinov2_backbones.__dict__[model_base_name]
    except KeyError as exc:
        raise ValueError(f"Unsupported DINOv2 backbone: {model_base_name}") from exc
...
class DinoFeatureExtractor(nn.Module):
    ...
    def __init__(self, model_name: str) -> None:
        ...
        self.model = load_dinov2_backbone(self.model_base_name)
