...
def _get_dataset_key(dataset_name):
    if dataset_name.startswith("electric_drill"):
        return "electric_drill"
    return dataset_name
...
def get_model_params(datasets_path, dataset_name, model_type=None):
    ...
    if dataset_key == "tless" and model_type is None:
        model_type = "cad"
...
def get_split_params(datasets_path, dataset_name, split, split_type=None):
    ...
    elif dataset_key == "electric_drill":
        p["scene_ids"] = {"train": [], "val": [1], "test": [1]}[split]
        p["im_size"] = (848, 480)

        if split == "test":
            p["depth_range"] = (100.0, 5000.0)
            p["azimuth_range"] = (0, 2 * math.pi)
            p["elev_range"] = (-0.5 * math.pi, 0.5 * math.pi)

    # HOT3D.
    elif dataset_key == "hot3d":
